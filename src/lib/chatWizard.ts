// Why: tiny session helper for conversational onboarding
export type AnswerStyle = 'eli5'|'intermediate'|'developer';
export type Message = { role:'user'|'assistant'; text:string; ts:number };
export type Answers = { idea?:string; name?:string; audience?:string; features?:string[]; privacy?:'Private'|'Share via link'|'Public'; auth?:'Google OAuth'|'Magic email link'|'None (dev only)'; deep_work_hours?:'0.5'|'1'|'2'|'4+' };
export type ChatSession = { step:number; answerStyle:AnswerStyle; answers:Answers; messages:Message[]; completed:boolean; timestamp:number };
const KEY = 'cp_chat_session_v1';
export const defaultSession = ():ChatSession => ({ step:0, answerStyle:'eli5', answers:{}, messages:[], completed:false, timestamp:Date.now() });
export const loadSession = ():ChatSession => { try { const j = localStorage.getItem(KEY); return j? JSON.parse(j): defaultSession(); } catch { return defaultSession(); } };
